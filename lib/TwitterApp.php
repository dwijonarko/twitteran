...
class TwitterApp {
    ...
    /**
     * Send a tweet with media on the user's behalf
     *
     * @param string $text Text to tweet
     * @return bool Tweet successfully sent
     */
    public function sendTweetImage($params) {
        // POST the text to the statuses/update method
        $code = $this->tmhOAuth->user_request(array(
            'method' => 'POST',
            'url' => $this->tmhOAuth->url("1.1/statuses/update_with_media"),
            'params' => $params,
            'multipart' => true
          ));
        return json_decode($this->tmhOAuth->response['response'],true);
    ...
    */
    public function getTweet(){
        $code = $this->tmhOAuth->request('GET', $this->tmhOAuth->url('1.1/statuses/home_timeline'));
        if ($code == 200) {
            $all = json_decode($this->tmhOAuth->response['response'],true);
            foreach($all as $row){
                $id = $row['id_str'];
                $text = $row['text'];
                ...
                echo $screen_name ."(".$name.")" .":".$text."<br>";
            }
        } else {
              tmhUtilities::pr(htmlentities($this->tmhOAuth->response['response']));
        }
    }
}